...
def seed_if_missing():
    ...
    if not F_NOTIF.exists():
        notif = pd.DataFrame([
            {"id": 1, "ts_creacion": now_iso(), "tag": "V-210", "titulo": "Chequear válvula V-210", "motivo": "Sobrepresión", "prioridad": "P1", "estado": "Pendiente", "asignado_a": "Operario 1", "ts_recibida": "", "ts_cerrada": "", "evidencia": ""},
            {"id": 2, "ts_creacion": now_iso(), "tag": "P-101", "titulo": "Verificar sello mecánico", "motivo": "Goteo observado", "prioridad": "P2", "estado": "Pendiente", "asignado_a": "Operario 2", "ts_recibida": "", "ts_cerrada": "", "evidencia": ""},
        ])
    ...
    if not F_INCIDENTES.exists():
        inc = pd.DataFrame([
            {"id": 1, "ts": now_iso(), "tag": "V-210", "titulo": "Near Miss por sobrepresión", "severidad": "Medio", "descripcion": "Se detecta lectura por encima del umbral", "reportado_por": "Operario 1", "estado": "Abierto"},
        ])
        save_csv(inc, F_INCIDENTES)

    if not F_PTWOT.exists():
        ptw = pd.DataFrame([
            {"id": 1, "ts_solicitud": now_iso(), "tipo": "Trabajo caliente", "solicitante": "Supervisor 1", "area": "Área B", "estado": "Borrador", "aprob_hse": "No", "ts_cierre": "", "adjuntos": ""},
        ])
...
if page == "Inicio":
    ...
    # KPIs simples
    k_total = len(notifs)
    k_pend = (notifs["estado"]=="Pendiente").sum() if not notifs.empty else 0
    k_rec  = (notifs["estado"]=="Recibida").sum()  if not notifs.empty else 0
    k_comp = (notifs["estado"]=="Completada").sum() if not notifs.empty else 0
    ...
    if not notifs.empty:
        st.dataframe(notifs.sort_values("ts_creacion", ascending=False).head(10), use_container_width=True)
    else:
...
# --------- NOTIFICACIONES ---------
    ...
    with st.expander("➕ Crear nueva notificación", expanded=True):
        ...
        with col3:
            ...
            if st.button("Crear notificación", use_container_width=True):
                new_id = 1 if not notifs.size else int(notifs["id"].max())+1
                new_row = {
                ...
                notifs.loc[len(notifs)] = new_row
                save_csv(notifs, F_NOTIF)
                st.success("Notificación creada.")
...
# --------- RONDAS ---------
    ...
    if rondas_plt.empty:
        ...
        for _, r in subset.iterrows():
            ...
            with col1:
                st.text_input("Tag", r["tag"], key=f"tag_{_}", disabled=True)
            with col2:
                st.text_input("Variable", r["variable"], key=f"var_{_}", disabled=True)
            with col3:
                valor = st.number_input("Valor", key=f"val_{_}", value=float((r["lim_inf"]+r["lim_sup"])/2))
            with col4:
                st.caption(f"Límites: [{r['lim_inf']}, {r['lim_sup']}]")
            en_rango = r["lim_inf"] <= valor <= r["lim_sup"]
            rows.append({
                "id": len(rondas_run) + len(rows) + 1,
...
# --------- PTW / OT ---------
    ...
    with col4:
        if st.button("Crear PTW", use_container_width=True):
            new_id = 1 if ptwot.empty else int(ptwot["id"].max())+1
            new_row = {
                "id": new_id, "ts_solicitud": now_iso(), "tipo": tipo,
                "solicitante": solicitante, "area": area, "estado": "Borrador",
                "aprob_hse": "No", "ts_cierre": "", "adjuntos": ""
            }
            ptwot.loc[len(ptwot)] = new_row
            save_csv(ptwot, F_PTWOT)
            st.success("PTW creado.")
...
# --------- INCIDENTES ---------
    ...
    if st.button("Reportar incidente"):
        new_id = 1 if incidentes.empty else int(incidentes["id"].max())+1
        new_row = {
        ...
        incidentes.loc[len(incidentes)] = new_row
...
# --------- DASHBOARD ---------
    ...
    with c1:
        st.markdown("#### KPIs")
        k_total = len(notifs)
        k_p1 = (notifs["prioridad"]=="P1").sum() if not notifs.empty else 0
        k_comp = (notifs["estado"]=="Completada").sum() if not notifs.empty else 0
        k_inc = len(incidentes)
    ...
    if not rondas_run.empty:
        ...
        fig2 = go.Figure()
        fig2.add_trace(go.Scatter(x=trend["date"], y=trend["en_rango"], mode="lines+markers", name="% en rango"))
...
# --------- CONFIG & IoT ---------
    ...
    st.caption("Simulación de regla: si P > P_high en V-210 ⇒ crear notificación P1 asignada a Operación Área B")
    col1, col2, col3 = st.columns(3)
    with col1:
    ...
    with col3:
        crear = st.button("Evaluar evento")

    if crear:
        if p_actual > p_high:
            new_id = 1 if notifs.empty else int(notifs["id"].max())+1
            new_row = {
            ...
            notifs.loc[len(notifs)] = new_row
            save_csv(notifs, F_NOTIF)
            st.success("Notificación P1 creada por evento IoT (simulado).")
        else:
